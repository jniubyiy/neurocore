// examples/adaptive_dropout_example.rs
// Пример обучения автоэнкодера с использованием слоя AdaptiveDropout.
// AdaptiveDropout — это dropout с вероятностью удержания, зависящей от величины
// активации. Для каждого признака есть обучаемые порог θ и температура T,
// определяющие, насколько вероятно сохранить сильные активации и отбросить слабые.
// В данном примере модель учится очищать зашумлённый сигнал: вход содержит
// два полезных признака с большой амплитудой и два шумовых с малой.
// Цель — восстановить полезные признаки и обнулить шумовые.
...
mod models {
    ...
    pub fn adaptive_dropout_denoiser() -> Vec<LayerDesc> {
        vec![
            ...
            // Адаптивный dropout с seed = 42
            LayerDesc::new(LayerKind::AdaptiveDropout)
...
/// Первые два признака — полезный сигнал с большой амплитудой (|x| > 1.0),
/// последние два — шум с малой амплитудой (|x| < 0.5).
/// Целевые значения: первые два признака равны входным, последние два — нули.
fn generate_data(num_samples: usize, seed: u64) -> (Tensor2D, Tensor2D) {
    ...
    for _ in 0..num_samples {
        ...
        // Полезные признаки: сильные значения
        for _ in 0..2 {
        ...
        // Шумовые признаки: слабые значения
        for _ in 0..2 {
...
fn base_training() -> neurocore::training_plan::TrainingPlan {
    ...
    TrainingPlan::new()
        .model(models::adaptive_dropout_denoiser)
