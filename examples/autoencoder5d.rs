...
fn main() {
    let descs = vec![
        LayerDesc::new("fc1", LayerKind::Linear, Dim::Dim5)
            .input(Dim::Dim5, &[4])
            .output(Dim::Dim5, &[2]),
        LayerDesc::new("sigm", LayerKind::Sigmoid, Dim::Dim5)
            .input(Dim::Dim5, &[2])
            .output(Dim::Dim5, &[2]),
        LayerDesc::new("fc2", LayerKind::Linear, Dim::Dim5)
            .input(Dim::Dim5, &[2])
            .output(Dim::Dim5, &[4]),
    ];
    ...
    for epoch in 0..epochs {
        let (pred_dyn, ctxs) = model.forward(DynamicTensor::Dim5(input.clone()));
    ...
    let (final_pred_dyn, _) = model.forward(DynamicTensor::Dim5(input.clone()));
