...
fn main() {
    ...
    // Вход: один 3D-образец (1 x 1 x 2)
    ...
    for epoch in 0..epochs {
        for (x, y) in &[(&x1, &y1), (&x2, &y2)] {
            let (pred_dyn, ctxs) = model.forward(DynamicTensor::Dim3((*x).clone()));
        ...
        if epoch % 50 == 0 {
            let (pred_dyn, _) = model.forward(DynamicTensor::Dim3(x1.clone()));
            let pred = match pred_dyn {
                DynamicTensor::Dim3(t) => t,
                _ => panic!(),
            };
