// examples/learnable_mish_example.rs
// Пример обучения автоэнкодера с использованием слоя LearnableMish.
// LearnableMish — это активация Mish с обучаемым параметром сглаживания λ.
// Модель учится восстанавливать входные данные, при этом λ адаптируется
// для оптимальной формы нелинейности.
...
mod models {
    ...
    pub fn learnable_mish_autoencoder() -> Vec<LayerDesc> {
        vec![
            ...
            // Активация LearnableMish с обучаемым параметром λ
            LayerDesc::new(LayerKind::LearnableMish)
...
/// (автоэнкодер). Восстановление таких данных требует гибкой нелинейности.
...
fn base_training() -> neurocore::training_plan::TrainingPlan {
    ...
    TrainingPlan::new()
        .model(models::learnable_mish_autoencoder)
