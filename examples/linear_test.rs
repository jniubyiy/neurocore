...
// Автоэнкодер Linear(4 -> 4), Dim1.
// Целевые значения равны входным (identity).
...
/// Генерирует 20 сэмплов автоэнкодера в [-1, 1]: target == input.
fn generate_data(num_samples: usize, seed: u64) -> (Tensor2D, Tensor2D) {
    ...
    for _ in 0..num_samples {
        let sample: Vec<f32> = (0..4).map(|_| rng.gen_range(-1.0..1.0)).collect();
