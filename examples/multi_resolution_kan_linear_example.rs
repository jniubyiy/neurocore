// examples/multi_resolution_kan_linear_example.rs
// Пример обучения модели аппроксимации функции с использованием слоя
// MultiResolutionKANLinear. Слой основан на KAN с двумя разрешениями сеток
// (грубая и точная), что позволяет улавливать как глобальные, так и локальные
// закономерности. В данном примере модель аппроксимирует функцию с
// низкочастотной и высокочастотной компонентами.
...
mod models {
    ...
    pub fn kan_approx_model() -> Vec<LayerDesc> {
        vec![
            // Первый KAN-слой: 1 вход -> 16 скрытых нейронов
            LayerDesc::new(LayerKind::MultiResolutionKANLinear)
                .input(shape!(batch, A[1]))
            ...
            // Второй KAN-слой: 16 входов -> 1 выход
            LayerDesc::new(LayerKind::MultiResolutionKANLinear)
                .input(shape!(batch, A[16]))
...
/// Генерирует обучающие данные: x из [-1, 1], y = sin(2πx) + 0.5*cos(10πx) + шум.
/// Такая функция содержит низкочастотную (sin) и высокочастотную (cos) компоненты,
/// что требует многомасштабной аппроксимации.
fn generate_data(num_samples: usize, seed: u64) -> (Tensor2D, Tensor2D) {
    ...
    for _ in 0..num_samples {
        ...
        let y = (2.0 * std::f32::consts::PI * x).sin()
            + 0.5 * (10.0 * std::f32::consts::PI * x).cos()
            + noise;
        inputs.push(vec![x]);
        targets.push(vec![y]);
    }
