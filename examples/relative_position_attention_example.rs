// examples/relative_position_attention_example.rs
// Пример обучения автоэнкодера с использованием слоя RelativePositionAttention.
// RelativePositionAttention — механизм внимания с обучаемым относительным
// позиционным смещением. Вход и выход имеют размерность (batch, seq_len * d_model).
// В данном примере модель учится восстанавливать входные последовательности.
...
mod models {
    ...
    pub fn relative_position_attention_autoencoder() -> Vec<LayerDesc> {
        let seq_len = 5;
        let d_model = 2;

        vec![
            // RelativePositionAttention с заданными seq_len и d_model
            LayerDesc::new(LayerKind::RelativePositionAttention)
                .input(shape!(batch, A[seq_len * d_model]))   // 10 признаков
                .output(shape!(batch, A[seq_len * d_model]))  // 10 признаков
...
/// Каждый пример — вектор из seq_len * d_model элементов.
...
fn base_training() -> neurocore::training_plan::TrainingPlan {
    ...
    TrainingPlan::new()
        .model(models::relative_position_attention_autoencoder)
