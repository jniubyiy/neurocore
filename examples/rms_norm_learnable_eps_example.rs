// examples/rms_norm_learnable_eps_example.rs
// Пример обучения автоэнкодера с использованием слоя RMSNormWithLearnableEpsilon.
// RMSNormWithLearnableEpsilon — RMS-нормализация с обучаемым параметром epsilon
// для каждого признака, что обеспечивает адаптивную численную стабильность.
// Модель учится восстанавливать входные данные, при этом epsilon подстраивается
// под статистику активаций.
...
mod models {
    ...
    pub fn rms_norm_eps_autoencoder() -> Vec<LayerDesc> {
        vec![
            ...
            // RMS-нормализация с обучаемым epsilon
            LayerDesc::new(LayerKind::RMSNormWithLearnableEpsilon)
...
/// (автоэнкодер). Восстановление таких данных требует стабильной нормализации.
...
fn base_training() -> neurocore::training_plan::TrainingPlan {
    ...
    TrainingPlan::new()
        .model(models::rms_norm_eps_autoencoder)
