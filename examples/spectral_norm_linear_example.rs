// examples/spectral_norm_linear_example.rs
// Пример обучения автоэнкодера с использованием слоя SpectrallyNormalizedLinear.
// SpectrallyNormalizedLinear — линейный слой со спектральной нормализацией весов
// и обучаемым масштабом. Ограничение спектральной нормы весовой матрицы
// стабилизирует обучение и предотвращает взрыв градиентов.
// В данном примере модель учится восстанавливать случайные векторы.
...
mod models {
    ...
    pub fn spectral_norm_autoencoder() -> Vec<LayerDesc> {
        vec![
            // Кодировщик с спектральной нормализацией
            LayerDesc::new(LayerKind::SpectrallyNormalizedLinear)
            ...
            // Декодер с спектральной нормализацией
            LayerDesc::new(LayerKind::SpectrallyNormalizedLinear)
                .input(shape!(batch, A[8]))
...
fn base_training() -> neurocore::training_plan::TrainingPlan {
    ...
    TrainingPlan::new()
        .model(models::spectral_norm_autoencoder)
