...
///
/// Поля `grad_handle`, `opt_state_handle` и `adapter_state_handle`
/// добавлены для корректной миграции всех буферов сегмента одним
/// проходом: если мигрировать только `params`, то при работе на GPU
/// `grads` останутся на CPU и следующий backward упадёт на assert
/// `grad_params_handle.is_gpu()`. Аналогично, `adapter_state_handle`
/// должен мигрировать синхронно с `params` (инвариант I-5:
...
pub struct SegmentTopologyInfo {
    ...
    /// Дескриптор буфера состояния градиентных адаптеров сегмента
    /// (MIGRATION_PLAN.md §7, Фаза 2).
    ///
    /// `None`, если у сегмента нет адаптеров с состоянием. Если адаптеры
    /// есть, этот буфер мигрирует **синхронно** с `param_handle`
    /// (инвариант I-5: устройство адаптера = устройство слоя).
    pub adapter_state_handle: Option<MatrixBufferHandle>,
