...
/// Процессор группы 1D слоёв. Использует общий ParamStore только для чтения параметров.
pub struct Dim1Processor {
...
impl Dim1Processor {
    ...
    pub fn forward(&self, input: &Tensor1D, params: &[f32]) -> (Tensor1D, Vec<LayerContext1D>) {
    ...
    fn forward_one(&self, input: &Tensor1D, params: &[f32]) -> (Tensor1D, Vec<LayerContext1D>) {
