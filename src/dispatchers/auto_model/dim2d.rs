...
/// Процессор группы 2D слоёв. Использует общий ParamStore только для чтения параметров.
pub struct Dim2Processor {
    pub layers: Vec<Box<dyn Layer2D>>,
...
impl Dim2Processor {
    ...
    pub fn forward(&self, input: &Tensor2D, params: &[f32]) -> (Tensor2D, Vec<LayerContext>) {
    ...
    fn forward_one(&self, input: &Tensor2D, params: &[f32]) -> (Tensor2D, Vec<LayerContext>) {
