...
/// Процессор группы 4D слоёв. Использует общий ParamStore только для чтения параметров.
pub struct Dim4Processor {
    pub layers: Vec<Box<dyn Layer4D>>,
...
impl Dim4Processor {
    ...
    pub fn forward(&self, input: &Tensor4D, params: &[f32]) -> (Tensor4D, Vec<LayerContext4D>) {
    ...
    fn forward_one(&self, input: &Tensor4D, params: &[f32]) -> (Tensor4D, Vec<LayerContext4D>) {
