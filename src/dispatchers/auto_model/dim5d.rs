...
/// Процессор группы 5D слоёв. Использует общий ParamStore только для чтения параметров.
pub struct Dim5Processor {
    pub layers: Vec<Box<dyn Layer5D>>,
...
impl Dim5Processor {
    ...
    pub fn forward(&self, input: &Tensor5D, params: &[f32]) -> (Tensor5D, Vec<LayerContext5D>) {
    ...
    fn forward_one(&self, input: &Tensor5D, params: &[f32]) -> (Tensor5D, Vec<LayerContext5D>) {
        ...
        for (layer, slice) in self.layers.iter().zip(&self.slices) {
            ...
            let dim4 = current[0].dim4;
            let mut bufs: Vec<Vec<Vec<Vec<Vec<Vec<f32>>>>>> = out_sizes
                .iter()
            ...
            current = bufs.into_iter().map(Tensor5D::new).collect();
        }
