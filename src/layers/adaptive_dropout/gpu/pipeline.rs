// src/layers/adaptive_dropout/gpu/pipeline.rs
use std::sync::Arc;
...
pub struct AdaptiveDropoutPipelines {
    pub forward: Arc<ComputePipeline>,
    pub backward: Arc<ComputePipeline>,
}

impl AdaptiveDropoutPipelines {
    pub fn new(device: Arc<Device>) -> Self {
        let fwd_bytes = include_bytes!("vulkan/shaders/adaptive_dropout_fwd.spv");
        let bwd_bytes = include_bytes!("vulkan/shaders/adaptive_dropout_bwd.spv");
        ...
        fn create_ds_layout(device: Arc<Device>, n: u32) -> Arc<DescriptorSetLayout> {
            ...
            .expect("Failed to create descriptor set layout for AdaptiveDropout")
        }
        ...
        let fwd_module = unsafe {
            ShaderModule::new(device.clone(), ShaderModuleCreateInfo::new(fwd_spv))
                .expect("Failed to create AdaptiveDropout forward shader module")
        };
        let fwd_push = PushConstantRange {
            stages: ShaderStages::COMPUTE,
            offset: 0,
            size: 12, // total (uint), features (uint), seed (uint)
        };
        ...
        .expect("Failed to create AdaptiveDropout forward pipeline layout");
        let fwd_entry = fwd_module
            .entry_point_with_execution("main", ExecutionModel::GLCompute)
            .expect("AdaptiveDropout forward entry point not found");
        ...
        .expect("Failed to create AdaptiveDropout forward pipeline");
        ...
        let bwd_module = unsafe {
            ShaderModule::new(device.clone(), ShaderModuleCreateInfo::new(bwd_spv))
                .expect("Failed to create AdaptiveDropout backward shader module")
        };
        let bwd_push = PushConstantRange {
            stages: ShaderStages::COMPUTE,
            offset: 0,
            size: 8, // total (uint), features (uint)
        };
        ...
        .expect("Failed to create AdaptiveDropout backward pipeline layout");
        let bwd_entry = bwd_module
            .entry_point_with_execution("main", ExecutionModel::GLCompute)
            .expect("AdaptiveDropout backward entry point not found");
        ...
        .expect("Failed to create AdaptiveDropout backward pipeline");
