...
impl GpuCompute {
    pub fn run_batch_renorm_forward_buffered_handle(
        ...
        let pipeline = &self.batch_renorm_pipelines().forward;
    ...
    pub fn run_batch_renorm_backward_buffered_handle(
        ...
        assert_eq!(params.len(), 4 * features);
        assert_eq!(grad_params.len(), 4 * features);
        ...
        let pipeline = &self.batch_renorm_pipelines().backward;
        let push = [batch as u32, features as u32];
        self.run_compute_shader(
            pipeline,
            &[
