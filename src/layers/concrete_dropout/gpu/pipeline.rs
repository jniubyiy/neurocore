// src/layers/concrete_dropout/gpu/pipeline.rs
use std::sync::Arc;
...
pub struct ConcreteDropoutPipelines {
    pub forward: Arc<ComputePipeline>,
    pub backward: Arc<ComputePipeline>,
}

impl ConcreteDropoutPipelines {
    pub fn new(device: Arc<Device>) -> Self {
        let fwd_bytes = include_bytes!("vulkan/shaders/concrete_dropout_fwd.spv");
        let bwd_bytes = include_bytes!("vulkan/shaders/concrete_dropout_bwd.spv");
        ...
        fn create_ds_layout(device: Arc<Device>, n: u32) -> Arc<DescriptorSetLayout> {
            ...
            .expect("Failed to create descriptor set layout for ConcreteDropout")
        }
        ...
        let fwd_module = unsafe {
            ShaderModule::new(device.clone(), ShaderModuleCreateInfo::new(fwd_spv))
                .expect("Failed to create ConcreteDropout forward shader module")
        };
        let fwd_push = PushConstantRange {
            stages: ShaderStages::COMPUTE,
            offset: 0,
            size: 12, // total (uint), temperature (float), seed (uint)
        };
        ...
        .expect("Failed to create ConcreteDropout forward pipeline layout");
        let fwd_entry = fwd_module
            .entry_point_with_execution("main", ExecutionModel::GLCompute)
            .expect("ConcreteDropout forward entry point not found");
        ...
        .expect("Failed to create ConcreteDropout forward pipeline");
        ...
        let bwd_module = unsafe {
            ShaderModule::new(device.clone(), ShaderModuleCreateInfo::new(bwd_spv))
                .expect("Failed to create ConcreteDropout backward shader module")
        };
        let bwd_push = PushConstantRange {
            stages: ShaderStages::COMPUTE,
            offset: 0,
            size: 8, // total (uint), temperature (float)
        };
        ...
        .expect("Failed to create ConcreteDropout backward pipeline layout");
        let bwd_entry = bwd_module
            .entry_point_with_execution("main", ExecutionModel::GLCompute)
            .expect("ConcreteDropout backward entry point not found");
        ...
        .expect("Failed to create ConcreteDropout backward pipeline");
