...
pub struct Memory4D { pub input_dim: usize, pub output_dim: usize }
impl Memory4D {
...
impl Layer4D for Memory4D {
    fn forward_into(&self, input: &Tensor4D, params: &[f32], slice: &ParamSlice, out_buf: &mut Vec<Vec<Vec<Vec<f32>>>>) -> LayerContext4D {
        let dim1 = input.dim1; let depth = input.depth; let rows = input.rows;
        ...
        for out_i in 0..self.output_dim {
            ...
            for d1 in 0..dim1 {
                for d in 0..depth {
                    for r in 0..rows {
                        out_buf[d1][d][r][out_i] = result_col[(idx, 0)];
                        idx += 1;
                    }
                }
            }
    ...
    fn backward(&self, ctx: &LayerContext4D, delta: &Tensor4D, params: &[f32], slice: &ParamSlice) -> (Tensor4D, Vec<f32>) {
        let input = match ctx { LayerContext4D::Memory4D { input } => input, _ => panic!() };
        let dim1 = delta.dim1; let depth = delta.depth; let rows = delta.rows;
        ...
        for d1 in 0..dim1 { for d in 0..depth { for r in 0..rows {
            for out_i in 0..self.output_dim {
                ...
                let delta_val = delta.data[d1][d][r][out_i];
                let base = out_i * (2 * self.input_dim + 1);
                for i in 0..self.input_dim {
                    grad[base + i] += delta_val * dy_dot0 * input.data[d1][d][r][i];
                    grad[base + self.input_dim + i] += delta_val * dy_dot1 * input.data[d1][d][r][i];
                }
                ...
                for i in 0..self.input_dim {
                    d_prev[d1][d][r][i] += delta_val * (dy_dot0 * m0[i] + dy_dot1 * m1[i]);
                }
            }
        }}}
