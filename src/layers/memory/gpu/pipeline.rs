// src/layers/memory/gpu/pipeline.rs
use std::sync::Arc;
...
impl MemoryPipelines {
    pub fn new(device: Arc<Device>) -> Self {
        // Загружаем SPIR-V
        let fwd_bytes = include_bytes!("vulkan/shaders/memory_fwd.spv");
        let bwd_bytes = include_bytes!("vulkan/shaders/memory_bwd.spv");
        ...
        fn create_ds_layout(device: Arc<Device>, n: u32) -> Arc<DescriptorSetLayout> {
            ...
            .expect("Failed to create descriptor set layout for Memory")
        }

        // Прямой пайплайн (3 буфера: input, state, output)
        ...
        let fwd_module = unsafe {
            ShaderModule::new(device.clone(), ShaderModuleCreateInfo::new(fwd_spv))
                .expect("Failed to create Memory forward shader module")
        };
        let fwd_push = PushConstantRange {
            stages: ShaderStages::COMPUTE,
            offset: 0,
            size: 12, // batch, features, alpha
        };
        ...
        .expect("Failed to create Memory forward pipeline layout");
        let fwd_entry = fwd_module
            .entry_point_with_execution("main", ExecutionModel::GLCompute)
            .expect("Memory forward entry point not found");
        ...
        .expect("Failed to create Memory forward pipeline");

        // Обратный пайплайн (2 буфера: grad_output, grad_input)
        ...
        let bwd_module = unsafe {
            ShaderModule::new(device.clone(), ShaderModuleCreateInfo::new(bwd_spv))
                .expect("Failed to create Memory backward shader module")
        };
        ...
        .expect("Failed to create Memory backward pipeline layout");
        let bwd_entry = bwd_module
            .entry_point_with_execution("main", ExecutionModel::GLCompute)
            .expect("Memory backward entry point not found");
        ...
        .expect("Failed to create Memory backward pipeline");
