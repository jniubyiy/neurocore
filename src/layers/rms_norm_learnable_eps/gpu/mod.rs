...
impl GpuCompute {
    pub fn run_rms_norm_learnable_eps_forward_buffered_handle(
        ...
        let pipeline = &self.rms_norm_learnable_eps_pipelines().forward;
    ...
    pub fn run_rms_norm_learnable_eps_backward_buffered_handle(
        ...
        assert_eq!(params.len(), 2 * features);
        assert_eq!(grad_params.len(), 2 * features);
        ...
        let pipeline = &self.rms_norm_learnable_eps_pipelines().backward;
        let push = [batch as u32, features as u32];
        self.run_compute_shader(
            pipeline,
            &[
