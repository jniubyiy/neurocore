// src/layers/sparse_feature_selection_gate/gpu/pipeline.rs
use std::sync::Arc;
...
pub struct SparseFeatureSelectionGatePipelines {
    pub forward: Arc<ComputePipeline>,
    pub backward: Arc<ComputePipeline>,
}

impl SparseFeatureSelectionGatePipelines {
    pub fn new(device: Arc<Device>) -> Self {
        let fwd_bytes = include_bytes!("vulkan/shaders/sparse_feature_selection_fwd.spv");
        let bwd_bytes = include_bytes!("vulkan/shaders/sparse_feature_selection_bwd.spv");
        ...
        fn create_ds_layout(device: Arc<Device>, n: u32) -> Arc<DescriptorSetLayout> {
            ...
            .expect("Failed to create descriptor set layout for SparseFeatureSelectionGate")
        }
        ...
        let fwd_module = unsafe {
            ShaderModule::new(device.clone(), ShaderModuleCreateInfo::new(fwd_spv))
                .expect("Failed to create SparseFeatureSelectionGate forward shader module")
        };
        ...
        .expect("Failed to create SparseFeatureSelectionGate forward pipeline layout");
        let fwd_entry = fwd_module
            .entry_point_with_execution("main", ExecutionModel::GLCompute)
            .expect("SparseFeatureSelectionGate forward entry point not found");
        ...
        .expect("Failed to create SparseFeatureSelectionGate forward pipeline");
        ...
        let bwd_module = unsafe {
            ShaderModule::new(device.clone(), ShaderModuleCreateInfo::new(bwd_spv))
                .expect("Failed to create SparseFeatureSelectionGate backward shader module")
        };
        ...
        .expect("Failed to create SparseFeatureSelectionGate backward pipeline layout");
        let bwd_entry = bwd_module
            .entry_point_with_execution("main", ExecutionModel::GLCompute)
            .expect("SparseFeatureSelectionGate backward entry point not found");
        ...
        .expect("Failed to create SparseFeatureSelectionGate backward pipeline");
