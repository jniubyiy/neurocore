...
/// Пайплайны Splitter.
///
/// Forward:  splitter_fwd — один поток на (r, j), j ∈ [0, p+q).
/// Backward: splitter_bwd_dx     — один поток на (r, c), c ∈ [0, n);
///           splitter_bwd_params — один поток на параметр.
...
impl SplitterPipelines {
    pub fn new(device: Arc<Device>) -> Self {
        ...
        let bwd_dx_bytes      = include_bytes!("vulkan/shaders/splitter_bwd_dx.spv");
        let bwd_params_bytes  = include_bytes!("vulkan/shaders/splitter_bwd_params.spv");
        ...
        // Все три пайплайна используют push = [batch, n, p, q] — 16 байт.
        const PUSH: u32 = 16;

        let forward         = build(device.clone(), fwd_spv,        9, PUSH, "Splitter forward");
        let backward_dx     = build(device.clone(), bwd_dx_spv,     7, PUSH, "Splitter bwd_dx");
        let backward_params = build(device.clone(), bwd_params_spv, 9, PUSH, "Splitter bwd_params");
