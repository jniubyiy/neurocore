// src/losses/log1p/gpu/pipeline.rs
use std::sync::Arc;
...
impl Log1pPipelines {
    pub fn new(device: Arc<Device>) -> Self {
        let fwd_bytes = include_bytes!("shaders/log1p_fwd.spv");
        let bwd_bytes = include_bytes!("shaders/log1p_bwd.spv");
        ...
        let fwd_module = unsafe {
            ShaderModule::new(device.clone(), ShaderModuleCreateInfo::new(fwd_spv))
                .expect("Failed to create Log1p forward shader module")
        };
        ...
        .expect("Failed to create Log1p forward pipeline layout");
        let fwd_entry = fwd_module
            .entry_point_with_execution("main", ExecutionModel::GLCompute)
            .expect("Log1p forward entry point not found");
        ...
        .expect("Failed to create Log1p forward pipeline");

        let bwd_module = unsafe {
            ShaderModule::new(device.clone(), ShaderModuleCreateInfo::new(bwd_spv))
                .expect("Failed to create Log1p backward shader module")
        };
        ...
        .expect("Failed to create Log1p backward pipeline layout");
        let bwd_entry = bwd_module
            .entry_point_with_execution("main", ExecutionModel::GLCompute)
            .expect("Log1p backward entry point not found");
        ...
        .expect("Failed to create Log1p backward pipeline");
...
fn create_ds_layout(device: Arc<Device>, n: u32) -> Arc<DescriptorSetLayout> {
    ...
    .expect("Failed to create descriptor set layout for Log1p")
}