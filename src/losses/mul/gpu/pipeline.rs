// src/losses/mul/gpu/pipeline.rs
use std::sync::Arc;
...
impl MulPipelines {
    pub fn new(device: Arc<Device>) -> Self {
        let fwd_bytes = include_bytes!("shaders/mul_fwd.spv");
        let bwd_bytes = include_bytes!("shaders/mul_bwd.spv");
        ...
        let fwd_module = unsafe {
            ShaderModule::new(device.clone(), ShaderModuleCreateInfo::new(fwd_spv))
                .expect("Failed to create Mul forward shader module")
        };
        ...
        .expect("Failed to create Mul forward pipeline layout");
        let fwd_entry = fwd_module
            .entry_point_with_execution("main", ExecutionModel::GLCompute)
            .expect("Mul forward entry point not found");
        ...
        .expect("Failed to create Mul forward pipeline");

        let bwd_module = unsafe {
            ShaderModule::new(device.clone(), ShaderModuleCreateInfo::new(bwd_spv))
                .expect("Failed to create Mul backward shader module")
        };
        ...
        .expect("Failed to create Mul backward pipeline layout");
        let bwd_entry = bwd_module
            .entry_point_with_execution("main", ExecutionModel::GLCompute)
            .expect("Mul backward entry point not found");
        ...
        .expect("Failed to create Mul backward pipeline");
...
fn create_ds_layout(device: Arc<Device>, n: u32) -> Arc<DescriptorSetLayout> {
    ...
    .expect("Failed to create descriptor set layout for Mul")
}