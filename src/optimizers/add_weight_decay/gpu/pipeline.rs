// src/optimizers/add_weight_decay/gpu/pipeline.rs
use std::sync::Arc;
...
pub struct AddWeightDecayPipelines {
    pub forward: Arc<ComputePipeline>,
}

impl AddWeightDecayPipelines {
    pub fn new(device: Arc<Device>) -> Self {
        let fwd_bytes = include_bytes!("shaders/weight_decay.spv");
        ...
        let fwd_module = unsafe {
            ShaderModule::new(device.clone(), ShaderModuleCreateInfo::new(fwd_spv))
                .expect("Failed to create AddWeightDecay shader module")
        };
        ...
        .expect("Failed to create AddWeightDecay pipeline layout");
        let fwd_entry = fwd_module
            .entry_point_with_execution("main", ExecutionModel::GLCompute)
            .expect("AddWeightDecay entry point not found");
        ...
        .expect("Failed to create AddWeightDecay pipeline");
    ...
    fn create_ds_layout(device: Arc<Device>, n: u32) -> Arc<DescriptorSetLayout> {
        ...
        .expect("Failed to create descriptor set layout for AddWeightDecay")
    }
}